...
This script initializes a basic Flask app with one route (/task1), which will return "Hello DevOps team" when accessed.
...
At this point, your app will be accessible at http://<VM_PUBLIC_IP>:5001/task1.
...
```bash
[Unit]
Description=Gunicorn instance to serve task1
...
sudo systemctl enable task1  # Enable the service to start on boot
sudo systemctl start task1  # Start the service immediately
```
...
```bash
sudo systemctl status app1  # Check the status of the task1 service
```


You should see something like:
```bash
● app1.service - Gunicorn instance to serve task1
...
```bash
Go to http://<VM_PUBLIC_IP>:5001/task1.
```
